    ...
    .factory('playGame', ['$http',
        ...
        function ($http, $rootScope, chatSocket, sharedProperties) {
            ...
            playGame.canDiscard = function (scope, card, melds) {
                ...
                var meldIndex;
                for (meldIndex = 0; meldIndex < scope.teams[0].melds.length; meldIndex++) {
                    if (scope.teams[0].melds[meldIndex].type !== 'Red Three') {
                        nowHasMelds = true;
                        break;
                    }
                }
                if (!scope.control.hasMelds && nowHasMelds && !melds.layDownScoreMet(scope))
                    return "Minimum score for lay down not yet met.";

                // can discard a black three
                ...
                for (meldIndex = 0; meldIndex < scope.teams[0].melds.length; meldIndex++) {
                    ...
                    if (meld.isComplete || meld.type === "Red Three" || meld.type === "Wild Card Meld") {
                        continue;
                    }
