export default class HandUtil {
    static dealNewHand(gameData) {
        ...
        const pileMax = Math.floor(allCards.length / 4);
        for (let pileIndex = 0; pileIndex < 4; pileIndex++) {
            for (let cardIndex = 0; cardIndex < pileMax; cardIndex++) {
                const cardPosition = Math.floor(Math.random() * allCards.length);
